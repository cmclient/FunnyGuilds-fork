...
public class IndividualNameTag {
    ...
    public void updatePlayer(Option<Player> targetPlayerOption, User targetUser) {
        ...
        String color = targetPlayer.isOp() ? nameTagConfig.nameColors.get("op") : null;
        if (color == null) {
            color = nameTagConfig.nameColors.entrySet().stream()
                    .filter(entry -> !entry.getKey().equals("op"))
        ...
        String prefix = this.prepareValue(this.prepareConfigValue(nameTagConfig.prefix, targetUser), targetPlayer, targetUser);
        if (color != null) {
            prefix += ChatUtils.colored(color);
        }
